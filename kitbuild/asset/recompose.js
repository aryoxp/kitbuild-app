  ...
  handleEvent() {
    ...
    $(".app-navbar").on("click", ".bt-reset", (e) => {
      ...
      let confirm = UI.confirm(
        ...
        .positive(() => {
          KitBuildApp.parseKitMapOptions(KitBuildApp.inst.kitMap);
          KitBuildApp.resetMapToKit(KitBuildApp.inst.kitMap, this.canvas);
    ...
    api.openFileResult((e, file) => {
      if (file === false) return;
      console.warn(file);
      if (!file.cmap) console.error("Invalid concept map.");
      if (!file.kit) console.error("Invalid kit.");
      let cmap = Core.decompress(file.cmap);
      let kit = Core.decompress(file.kit);
      kit.conceptMap = cmap;
      KitBuildApp.parseKitMapOptions(kit);
      this.setKitMap(kit);
      this.setConceptMap(cmap);
      console.warn(cmap, kit);
      this.showKit(cmap, kit);
    });

    api.getLoadedFileResult((e, file) => {
